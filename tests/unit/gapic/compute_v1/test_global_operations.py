...
from google.cloud.compute_v1.services.global_operations.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.global_operations.transports.base import _GOOGLE_AUTH_VERSION
...
def test_global_operations_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListGlobalOperationsRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.OperationAggregatedList(
            id='id_value',
            items={'key_value': compute.OperationsScopedList(operations=[compute.Operation(client_operation_id='client_operation_id_value')])},
            kind='kind_value',
    ...
    assert response.items == {'key_value': compute.OperationsScopedList(operations=[compute.Operation(client_operation_id='client_operation_id_value')])}
...
def test_global_operations_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.global_operations.transports.GlobalOperationsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_global_operations_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.global_operations.transports.GlobalOperationsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.GlobalOperationsTransport()
        adc.assert_called_once()
