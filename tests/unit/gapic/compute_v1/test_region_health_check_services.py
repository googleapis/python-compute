...
from google.cloud.compute_v1.services.region_health_check_services.transports.base import (
...
def test_region_health_check_services_client_service_account_always_use_jwt_true(
    transport_class, transport_name
):
    with mock.patch.object(
        service_account.Credentials, "with_always_use_jwt_access", create=True
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        assert "health_check_service_value" in http_call[1] + str(body)
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        client.insert(
            project="project_value",
            region="region_value",
            health_check_service_resource=health_check_service_resource,
        )
        ...
        assert "project_value" in http_call[1] + str(body)
        assert "region_value" in http_call[1] + str(body)
        assert compute.HealthCheckService.to_json(
...
def test_list_rest(
    ...
    assert response.items == [
        compute.HealthCheckService(creation_timestamp="creation_timestamp_value")
    ]
    assert response.kind == "kind_value"
