...
from google.cloud.compute_v1.services.ssl_certificates.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.ssl_certificates.transports.base import _GOOGLE_AUTH_VERSION
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListSslCertificatesRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.SslCertificateAggregatedList(
            id='id_value',
            items={'key_value': compute.SslCertificatesScopedList(ssl_certificates=[compute.SslCertificate(certificate='certificate_value')])},
            kind='kind_value',
    ...
    assert response.items == {'key_value': compute.SslCertificatesScopedList(ssl_certificates=[compute.SslCertificate(certificate='certificate_value')])}
...
def test_ssl_certificates_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.ssl_certificates.transports.SslCertificatesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_ssl_certificates_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.ssl_certificates.transports.SslCertificatesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (credentials.AnonymousCredentials(), None)
...
def test_ssl_certificates_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.ssl_certificates.transports.SslCertificatesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.SslCertificatesTransport()
        adc.assert_called_once()
...
@requires_google_auth_lt_1_25_0
def test_ssl_certificates_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(auth, 'default', autospec=True) as adc:
        adc.return_value = (credentials.AnonymousCredentials(), None)
        SslCertificatesClient()
