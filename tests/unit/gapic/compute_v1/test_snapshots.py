...
from google.cloud.compute_v1.services.snapshots.transports.base import _GOOGLE_AUTH_VERSION
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetSnapshotRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.Snapshot(
            ...
            name='name_value',
            self_link='self_link_value',
            snapshot_encryption_key=compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value'),
    ...
    assert response.name == 'name_value'
    assert response.self_link == 'self_link_value'
    assert response.snapshot_encryption_key == compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value')
...
def test_snapshots_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.snapshots.transports.SnapshotsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_snapshots_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.snapshots.transports.SnapshotsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.SnapshotsTransport()
        adc.assert_called_once()
