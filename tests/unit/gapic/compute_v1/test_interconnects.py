...
from google.cloud.compute_v1.services.interconnects import (
    InterconnectsClient,
    pagers,
    transports,
)
...
def test_insert_rest(request_type):
    ...
    request_init["interconnect_resource"] = {
        ...
        "expected_outages": [
            {
                "affected_circuits": [
                    "affected_circuits_value1",
                    "affected_circuits_value2",
                ],
                "description": "description_value",
                "end_time": 837,
...
def test_set_labels_rest(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.SetLabelsInterconnectRequest.meta.fields[
...
def test_set_labels_rest_required_fields(
    request_type=compute.SetLabelsInterconnectRequest,
):
    transport_class = transports.InterconnectsRestTransport
...
def test_set_labels_rest_bad_request(
    transport: str = "rest", request_type=compute.SetLabelsInterconnectRequest
):
    client = InterconnectsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
...
def test_set_labels_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/global/interconnects/{resource}/setLabels"
            % client.transport._host,
            args[1],
        )
