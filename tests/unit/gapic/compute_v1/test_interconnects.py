...
from google.cloud.compute_v1.services.interconnects.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.interconnects.transports.base import _GOOGLE_AUTH_VERSION
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetInterconnectRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.Interconnect(
            admin_enabled=True,
            circuit_infos=[compute.InterconnectCircuitInfo(customer_demarc_id='customer_demarc_id_value')],
            creation_timestamp='creation_timestamp_value',
            customer_name='customer_name_value',
            description='description_value',
            expected_outages=[compute.InterconnectOutageNotification(affected_circuits=['affected_circuits_value'])],
    ...
    assert response.circuit_infos == [compute.InterconnectCircuitInfo(customer_demarc_id='customer_demarc_id_value')]
    assert response.creation_timestamp == 'creation_timestamp_value'
    assert response.customer_name == 'customer_name_value'
    assert response.description == 'description_value'
    assert response.expected_outages == [compute.InterconnectOutageNotification(affected_circuits=['affected_circuits_value'])]
...
def test_get_diagnostics_rest(transport: str = 'rest', request_type=compute.GetDiagnosticsInterconnectRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.InterconnectsGetDiagnosticsResponse(
            result=compute.InterconnectDiagnostics(arp_caches=[compute.InterconnectDiagnosticsARPEntry(ip_address='ip_address_value')]),
        )
    ...
    assert response.result == compute.InterconnectDiagnostics(arp_caches=[compute.InterconnectDiagnosticsARPEntry(ip_address='ip_address_value')])
...
project='project_value',interconnect_resource=interconnect_resource,        )
        ...
        assert 'project_value'
 in http_call[1] + str(body)
        assert compute.Interconnect.to_json(interconnect_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_patch_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        interconnect_resource = compute.Interconnect(admin_enabled=True)
        client.patch(
project='project_value',interconnect='interconnect_value',interconnect_resource=interconnect_resource,        )
...
def test_interconnects_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.interconnects.transports.InterconnectsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_interconnects_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.interconnects.transports.InterconnectsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (credentials.AnonymousCredentials(), None)
...
def test_interconnects_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.interconnects.transports.InterconnectsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.InterconnectsTransport()
        adc.assert_called_once()
...
@requires_google_auth_lt_1_25_0
def test_interconnects_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(auth, 'default', autospec=True) as adc:
        adc.return_value = (credentials.AnonymousCredentials(), None)
        InterconnectsClient()
