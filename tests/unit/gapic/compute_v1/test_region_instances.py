...
from google.cloud.compute_v1.services.region_instances.transports.base import (
...
@pytest.mark.parametrize("client_class", [RegionInstancesClient,])
def test_region_instances_client_service_account_always_use_jwt(client_class):
    with mock.patch.object(
...
def test_region_instances_client_service_account_always_use_jwt_true(
    transport_class, transport_name
):
    with mock.patch.object(
        service_account.Credentials, "with_always_use_jwt_access", create=True
...
def test_bulk_insert_rest_flattened():
    client = RegionInstancesClient(credentials=ga_credentials.AnonymousCredentials(),)
...
@requires_google_auth_lt_1_25_0
def test_region_instances_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        RegionInstancesClient()
