...
from google.cloud.compute_v1.services.disk_types.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.disk_types.transports.base import _GOOGLE_AUTH_VERSION
...
def test_disk_types_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListDiskTypesRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.DiskTypeAggregatedList(
            id='id_value',
            items={'key_value': compute.DiskTypesScopedList(disk_types=[compute.DiskType(creation_timestamp='creation_timestamp_value')])},
            kind='kind_value',
    ...
    assert response.items == {'key_value': compute.DiskTypesScopedList(disk_types=[compute.DiskType(creation_timestamp='creation_timestamp_value')])}
...
def test_disk_types_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.disk_types.transports.DiskTypesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_disk_types_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.disk_types.transports.DiskTypesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.DiskTypesTransport()
        adc.assert_called_once()
