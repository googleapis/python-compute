...
def test_add_resource_policies_rest(transport: str = 'rest', request_type=compute.AddResourcePoliciesDiskRequest):
    client = DisksClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
project='project_value',zone='zone_value',disk='disk_value',disks_add_resource_policies_request_resource=disks_add_resource_policies_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'disk_value'
 in http_call[1] + str(body)
        assert compute.DisksAddResourcePoliciesRequest.to_json(disks_add_resource_policies_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListDisksRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.DiskAggregatedList(
            id='id_value',
            items={'key_value': compute.DisksScopedList(disks=[compute.Disk(creation_timestamp='creation_timestamp_value')])},
            kind='kind_value',
    ...
    assert response.id == 'id_value'
    assert response.items == {'key_value': compute.DisksScopedList(disks=[compute.Disk(creation_timestamp='creation_timestamp_value')])}
    assert response.kind == 'kind_value'
...
def test_create_snapshot_rest(transport: str = 'rest', request_type=compute.CreateSnapshotDiskRequest):
    client = DisksClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
project='project_value',zone='zone_value',disk='disk_value',snapshot_resource=snapshot_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'disk_value'
 in http_call[1] + str(body)
        assert compute.Snapshot.to_json(snapshot_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetDiskRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.Disk(
            creation_timestamp='creation_timestamp_value',
            description='description_value',
            disk_encryption_key=compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value'),
            guest_os_features=[compute.GuestOsFeature(type_=compute.GuestOsFeature.Type.FEATURE_TYPE_UNSPECIFIED)],
            id='id_value',
            kind='kind_value',
            ...
            source_image_encryption_key=compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value'),
            source_image_id='source_image_id_value',
            source_snapshot='source_snapshot_value',
            source_snapshot_encryption_key=compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value'),
    ...
    assert response.creation_timestamp == 'creation_timestamp_value'
    assert response.description == 'description_value'
    assert response.disk_encryption_key == compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value')
    assert response.guest_os_features == [compute.GuestOsFeature(type_=compute.GuestOsFeature.Type.FEATURE_TYPE_UNSPECIFIED)]
    assert response.id == 'id_value'
    assert response.kind == 'kind_value'
    ...
    assert response.name == 'name_value'
    assert response.options_ == 'options__value'
    ...
    assert response.source_image_encryption_key == compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value')
    assert response.source_image_id == 'source_image_id_value'
    assert response.source_snapshot == 'source_snapshot_value'
    assert response.source_snapshot_encryption_key == compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value')
...
project='project_value',zone='zone_value',disk_resource=disk_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'zone_value'
 in http_call[1] + str(body)
        assert compute.Disk.to_json(disk_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListDisksRequest):
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.Disk(creation_timestamp='creation_timestamp_value')]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_remove_resource_policies_rest(transport: str = 'rest', request_type=compute.RemoveResourcePoliciesDiskRequest):
    client = DisksClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
project='project_value',zone='zone_value',disk='disk_value',disks_remove_resource_policies_request_resource=disks_remove_resource_policies_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'disk_value'
 in http_call[1] + str(body)
        assert compute.DisksRemoveResourcePoliciesRequest.to_json(disks_remove_resource_policies_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_resize_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        disks_resize_request_resource = compute.DisksResizeRequest(size_gb='size_gb_value')
        client.resize(
project='project_value',zone='zone_value',disk='disk_value',disks_resize_request_resource=disks_resize_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'disk_value'
 in http_call[1] + str(body)
        assert compute.DisksResizeRequest.to_json(disks_resize_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_set_labels_rest(transport: str = 'rest', request_type=compute.SetLabelsDiskRequest):
    client = DisksClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_set_labels_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.set_labels(
project='project_value',zone='zone_value',resource='resource_value',zone_set_labels_request_resource=zone_set_labels_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'resource_value'
 in http_call[1] + str(body)
        assert compute.ZoneSetLabelsRequest.to_json(zone_set_labels_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_disks_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.disks.transports.DisksTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_disks_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.disks.transports.DisksTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.DisksTransport()
        adc.assert_called_once()
