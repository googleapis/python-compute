...
from google.cloud.compute_v1.services.public_delegated_prefixes.transports.base import (
...
@pytest.mark.parametrize("client_class", [PublicDelegatedPrefixesClient,])
def test_public_delegated_prefixes_client_service_account_always_use_jwt(client_class):
    with mock.patch.object(
...
def test_public_delegated_prefixes_client_service_account_always_use_jwt_true(
    transport_class, transport_name
):
    with mock.patch.object(
        service_account.Credentials, "with_always_use_jwt_access", create=True
...
def test_aggregated_list_rest(
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        return_value = compute.PublicDelegatedPrefixAggregatedList(
            id="id_value",
            items={
                "key_value": compute.PublicDelegatedPrefixesScopedList(
        ...
        json_return_value = compute.PublicDelegatedPrefixAggregatedList.to_json(
            return_value
        )
...
def test_public_delegated_prefixes_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(
        google.auth, "load_credentials_from_file", autospec=True
    ) as load_creds, mock.patch(
...
@requires_google_auth_lt_1_25_0
def test_public_delegated_prefixes_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        PublicDelegatedPrefixesClient()
