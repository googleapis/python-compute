...
from google.cloud.compute_v1.services.backend_services.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.backend_services.transports.base import _GOOGLE_AUTH_VERSION
...
project='project_value',backend_service='backend_service_value',signed_url_key_resource=signed_url_key_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'backend_service_value'
 in http_call[1] + str(body)
        assert compute.SignedUrlKey.to_json(signed_url_key_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListBackendServicesRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.BackendServiceAggregatedList(
            id='id_value',
            items={'key_value': compute.BackendServicesScopedList(backend_services=[compute.BackendService(affinity_cookie_ttl_sec=2432)])},
            kind='kind_value',
    ...
    assert response.id == 'id_value'
    assert response.items == {'key_value': compute.BackendServicesScopedList(backend_services=[compute.BackendService(affinity_cookie_ttl_sec=2432)])}
    assert response.kind == 'kind_value'
...
project='project_value',backend_service='backend_service_value',key_name='key_name_value',        )
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetBackendServiceRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.BackendService(
            ...
            backends=[compute.Backend(balancing_mode=compute.Backend.BalancingMode.CONNECTION)],
            cdn_policy=compute.BackendServiceCdnPolicy(cache_key_policy=compute.CacheKeyPolicy(include_host=True)),
            ...
            consistent_hash=compute.ConsistentHashLoadBalancerSettings(http_cookie=compute.ConsistentHashLoadBalancerSettingsHttpCookie(name='name_value')),
            creation_timestamp='creation_timestamp_value',
            custom_request_headers=['custom_request_headers_value'],
            custom_response_headers=['custom_response_headers_value'],
            description='description_value',
            enable_c_d_n=True,
            failover_policy=compute.BackendServiceFailoverPolicy(disable_connection_drain_on_failover=True),
            ...
            outlier_detection=compute.OutlierDetection(base_ejection_time=compute.Duration(nanos=543)),
            port=453,
            port_name='port_name_value',
            protocol=compute.BackendService.Protocol.GRPC,
            region='region_value',
            security_policy='security_policy_value',
            security_settings=compute.SecuritySettings(client_tls_policy='client_tls_policy_value'),
            self_link='self_link_value',
    ...
    assert response.backends == [compute.Backend(balancing_mode=compute.Backend.BalancingMode.CONNECTION)]
    assert response.cdn_policy == compute.BackendServiceCdnPolicy(cache_key_policy=compute.CacheKeyPolicy(include_host=True))
    assert response.circuit_breakers == compute.CircuitBreakers(max_connections=1608)
    assert response.connection_draining == compute.ConnectionDraining(draining_timeout_sec=2124)
    assert response.consistent_hash == compute.ConsistentHashLoadBalancerSettings(http_cookie=compute.ConsistentHashLoadBalancerSettingsHttpCookie(name='name_value'))
    assert response.creation_timestamp == 'creation_timestamp_value'
    ...
    assert response.failover_policy == compute.BackendServiceFailoverPolicy(disable_connection_drain_on_failover=True)
    ...
    assert response.outlier_detection == compute.OutlierDetection(base_ejection_time=compute.Duration(nanos=543))
    ...
    assert response.security_settings == compute.SecuritySettings(client_tls_policy='client_tls_policy_value')
    assert response.self_link == 'self_link_value'
...
def test_get_health_rest(transport: str = 'rest', request_type=compute.GetHealthBackendServiceRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.BackendServiceGroupHealth(
            ...
            health_status=[compute.HealthStatus(annotations={'key_value': 'value_value'})],
            kind='kind_value',
        )
    ...
    assert response.health_status == [compute.HealthStatus(annotations={'key_value': 'value_value'})]
    assert response.kind == 'kind_value'
...
def test_get_health_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.get_health(
project='project_value',backend_service='backend_service_value',resource_group_reference_resource=resource_group_reference_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'backend_service_value'
 in http_call[1] + str(body)
        assert compute.ResourceGroupReference.to_json(resource_group_reference_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
project='project_value',backend_service_resource=backend_service_resource,        )
        ...
        assert 'project_value'
 in http_call[1] + str(body)
        assert compute.BackendService.to_json(backend_service_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_patch_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        backend_service_resource = compute.BackendService(affinity_cookie_ttl_sec=2432)
        client.patch(
project='project_value',backend_service='backend_service_value',backend_service_resource=backend_service_resource,        )
...
project='project_value',backend_service='backend_service_value',security_policy_reference_resource=security_policy_reference_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'backend_service_value'
 in http_call[1] + str(body)
        assert compute.SecurityPolicyReference.to_json(security_policy_reference_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_backend_services_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.backend_services.transports.BackendServicesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_backend_services_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.backend_services.transports.BackendServicesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.BackendServicesTransport()
        adc.assert_called_once()
