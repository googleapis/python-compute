...
from google.cloud.compute_v1.services.interconnect_attachments.transports.base import (
...
@pytest.mark.parametrize("client_class", [InterconnectAttachmentsClient,])
def test_interconnect_attachments_client_service_account_always_use_jwt(client_class):
    with mock.patch.object(
...
def test_interconnect_attachments_client_service_account_always_use_jwt_true(
    transport_class, transport_name
):
    with mock.patch.object(
        service_account.Credentials, "with_always_use_jwt_access", create=True
