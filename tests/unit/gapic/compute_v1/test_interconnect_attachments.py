...
from google.cloud.compute_v1.services.interconnect_attachments import InterconnectAttachmentsClient
...
from google.cloud.compute_v1.services.interconnect_attachments.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.interconnect_attachments.transports.base import _GOOGLE_AUTH_VERSION
...
def test_interconnect_attachments_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (InterconnectAttachmentsClient, transports.InterconnectAttachmentsRestTransport, "rest"),
])
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (InterconnectAttachmentsClient, transports.InterconnectAttachmentsRestTransport, "rest", "true"),
    (InterconnectAttachmentsClient, transports.InterconnectAttachmentsRestTransport, "rest", "false"),
])
@mock.patch.object(InterconnectAttachmentsClient, "DEFAULT_ENDPOINT", modify_default_endpoint(InterconnectAttachmentsClient))
...
def test_interconnect_attachments_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListInterconnectAttachmentsRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.InterconnectAttachmentAggregatedList(
            id='id_value',
            items={'key_value': compute.InterconnectAttachmentsScopedList(interconnect_attachments=[compute.InterconnectAttachment(admin_enabled=True)])},
            kind='kind_value',
        ...
        json_return_value = compute.InterconnectAttachmentAggregatedList.to_json(return_value)
    ...
    assert response.items == {'key_value': compute.InterconnectAttachmentsScopedList(interconnect_attachments=[compute.InterconnectAttachment(admin_enabled=True)])}
...
def test_delete_rest(transport: str = 'rest', request_type=compute.DeleteInterconnectAttachmentRequest):
    client = InterconnectAttachmentsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # using the keyword arguments to the method.
        client.delete(
project='project_value',region='region_value',interconnect_attachment='interconnect_attachment_value',        )
 ...
 in http_call[1] + str(body)
        assert 'region_value'
 in http_call[1] + str(body)
        assert 'interconnect_attachment_value'
 in http_call[1] + str(body)
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetInterconnectAttachmentRequest):
    client = InterconnectAttachmentsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.InterconnectAttachment(
            ...
            partner_metadata=compute.InterconnectAttachmentPartnerMetadata(interconnect_name='interconnect_name_value'),
            private_interconnect_info=compute.InterconnectAttachmentPrivateInfo(tag8021q=632),
            region='region_value',
            router='router_value',
            self_link='self_link_value',
    ...
    assert response.description == 'description_value'
    assert response.edge_availability_domain == compute.InterconnectAttachment.EdgeAvailabilityDomain.AVAILABILITY_DOMAIN_1
    ...
    assert response.name == 'name_value'
    assert response.operational_status == compute.InterconnectAttachment.OperationalStatus.OS_ACTIVE
    assert response.pairing_key == 'pairing_key_value'
    assert response.partner_asn == 'partner_asn_value'
    assert response.partner_metadata == compute.InterconnectAttachmentPartnerMetadata(interconnect_name='interconnect_name_value')
    assert response.private_interconnect_info == compute.InterconnectAttachmentPrivateInfo(tag8021q=632)
    assert response.region == 'region_value'
...
def test_insert_rest(transport: str = 'rest', request_type=compute.InsertInterconnectAttachmentRequest):
    client = InterconnectAttachmentsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        interconnect_attachment_resource = compute.InterconnectAttachment(admin_enabled=True)
        client.insert(
project='project_value',region='region_value',interconnect_attachment_resource=interconnect_attachment_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'region_value'
 in http_call[1] + str(body)
        assert compute.InterconnectAttachment.to_json(interconnect_attachment_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListInterconnectAttachmentsRequest):
    client = InterconnectAttachmentsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_pager():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, compute.InterconnectAttachment)
                   for i in results)
...
def test_patch_rest(transport: str = 'rest', request_type=compute.PatchInterconnectAttachmentRequest):
    client = InterconnectAttachmentsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_patch_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        interconnect_attachment_resource = compute.InterconnectAttachment(admin_enabled=True)
        client.patch(
project='project_value',region='region_value',interconnect_attachment='interconnect_attachment_value',interconnect_attachment_resource=interconnect_attachment_resource,        )
...
def test_interconnect_attachments_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.interconnect_attachments.transports.InterconnectAttachmentsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_interconnect_attachments_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.interconnect_attachments.transports.InterconnectAttachmentsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (credentials.AnonymousCredentials(), None)
...
def test_interconnect_attachments_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.interconnect_attachments.transports.InterconnectAttachmentsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.InterconnectAttachmentsTransport()
        adc.assert_called_once()
...
@requires_google_auth_lt_1_25_0
def test_interconnect_attachments_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(auth, 'default', autospec=True) as adc:
        adc.return_value = (credentials.AnonymousCredentials(), None)
        InterconnectAttachmentsClient()
