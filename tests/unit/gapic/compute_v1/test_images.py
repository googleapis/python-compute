...
def test_deprecate_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.deprecate(
project='project_value',image='image_value',deprecation_status_resource=deprecation_status_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'image_value'
 in http_call[1] + str(body)
        assert compute.DeprecationStatus.to_json(deprecation_status_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetImageRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.Image(
            ...
            id='id_value',
            image_encryption_key=compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value'),
            kind='kind_value',
            ...
            self_link='self_link_value',
            shielded_instance_initial_state=compute.InitialStateConfig(dbs=[compute.FileContentBuffer(content='content_value')]),
            source_disk='source_disk_value',
            source_disk_encryption_key=compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value'),
    ...
    assert response.id == 'id_value'
    assert response.image_encryption_key == compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value')
    assert response.kind == 'kind_value'
    ...
    assert response.name == 'name_value'
    assert response.raw_disk == compute.RawDisk(container_type=compute.RawDisk.ContainerType.TAR)
    assert response.self_link == 'self_link_value'
    assert response.shielded_instance_initial_state == compute.InitialStateConfig(dbs=[compute.FileContentBuffer(content='content_value')])
    assert response.source_disk == 'source_disk_value'
    assert response.source_disk_encryption_key == compute.CustomerEncryptionKey(kms_key_name='kms_key_name_value')
...
project='project_value',image_resource=image_resource,        )
        ...
        assert 'project_value'
 in http_call[1] + str(body)
        assert compute.Image.to_json(image_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
project='project_value',resource='resource_value',global_set_policy_request_resource=global_set_policy_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'resource_value'
 in http_call[1] + str(body)
        assert compute.GlobalSetPolicyRequest.to_json(global_set_policy_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_set_labels_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.set_labels(
project='project_value',resource='resource_value',global_set_labels_request_resource=global_set_labels_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'resource_value'
 in http_call[1] + str(body)
        assert compute.GlobalSetLabelsRequest.to_json(global_set_labels_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
project='project_value',resource='resource_value',test_permissions_request_resource=test_permissions_request_resource,        )
...
def test_images_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.images.transports.ImagesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_images_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.images.transports.ImagesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (credentials.AnonymousCredentials(), None)
...
def test_images_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.images.transports.ImagesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.ImagesTransport()
        adc.assert_called_once()
...
@requires_google_auth_lt_1_25_0
def test_images_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(auth, 'default', autospec=True) as adc:
        adc.return_value = (credentials.AnonymousCredentials(), None)
        ImagesClient()
