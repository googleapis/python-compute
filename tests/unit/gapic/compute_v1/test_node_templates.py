...
from google.cloud.compute_v1.services.node_templates.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.node_templates.transports.base import _GOOGLE_AUTH_VERSION
...
def test_node_templates_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListNodeTemplatesRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeTemplateAggregatedList(
            id='id_value',
            items={'key_value': compute.NodeTemplatesScopedList(node_templates=[compute.NodeTemplate(accelerators=[compute.AcceleratorConfig(accelerator_count=1805)])])},
            kind='kind_value',
    ...
    assert response.items == {'key_value': compute.NodeTemplatesScopedList(node_templates=[compute.NodeTemplate(accelerators=[compute.AcceleratorConfig(accelerator_count=1805)])])}
...
def test_delete_rest(transport: str = 'rest', request_type=compute.DeleteNodeTemplateRequest):
    client = NodeTemplatesClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # using the keyword arguments to the method.
        client.delete(
project='project_value',region='region_value',node_template='node_template_value',        )
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetNodeTemplateRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeTemplate(
            ...
            node_type_flexibility=compute.NodeTemplateNodeTypeFlexibility(cpus='cpus_value'),
            region='region_value',
            self_link='self_link_value',
            server_binding=compute.ServerBinding(type_=compute.ServerBinding.Type.RESTART_NODE_ON_ANY_SERVER),
    ...
    assert response.cpu_overcommit_type == compute.NodeTemplate.CpuOvercommitType.CPU_OVERCOMMIT_TYPE_UNSPECIFIED
    assert response.creation_timestamp == 'creation_timestamp_value'
    assert response.description == 'description_value'
    ...
    assert response.node_type_flexibility == compute.NodeTemplateNodeTypeFlexibility(cpus='cpus_value')
    assert response.region == 'region_value'
    assert response.self_link == 'self_link_value'
    assert response.server_binding == compute.ServerBinding(type_=compute.ServerBinding.Type.RESTART_NODE_ON_ANY_SERVER)
...
def test_insert_rest(transport: str = 'rest', request_type=compute.InsertNodeTemplateRequest):
    client = NodeTemplatesClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.insert(
project='project_value',region='region_value',node_template_resource=node_template_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'region_value'
 in http_call[1] + str(body)
        assert compute.NodeTemplate.to_json(node_template_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListNodeTemplatesRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeTemplateList(
            id='id_value',
            items=[compute.NodeTemplate(accelerators=[compute.AcceleratorConfig(accelerator_count=1805)])],
            kind='kind_value',
            next_page_token='next_page_token_value',
            self_link='self_link_value',
            warning=compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED),
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.NodeTemplate(accelerators=[compute.AcceleratorConfig(accelerator_count=1805)])]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_node_templates_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.node_templates.transports.NodeTemplatesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_node_templates_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.node_templates.transports.NodeTemplatesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.NodeTemplatesTransport()
        adc.assert_called_once()
