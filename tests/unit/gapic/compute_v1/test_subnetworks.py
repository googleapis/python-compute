...
from google.cloud.compute_v1.services.subnetworks.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.subnetworks.transports.base import _GOOGLE_AUTH_VERSION
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListSubnetworksRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.SubnetworkAggregatedList(
            id='id_value',
            items={'key_value': compute.SubnetworksScopedList(subnetworks=[compute.Subnetwork(creation_timestamp='creation_timestamp_value')])},
            kind='kind_value',
    ...
    assert response.items == {'key_value': compute.SubnetworksScopedList(subnetworks=[compute.Subnetwork(creation_timestamp='creation_timestamp_value')])}
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # using the keyword arguments to the method.
        client.delete(
project='project_value',region='region_value',subnetwork='subnetwork_value',        )
...
project='project_value',region='region_value',subnetwork='subnetwork_value',subnetworks_expand_ip_cidr_range_request_resource=subnetworks_expand_ip_cidr_range_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'subnetwork_value'
 in http_call[1] + str(body)
        assert compute.SubnetworksExpandIpCidrRangeRequest.to_json(subnetworks_expand_ip_cidr_range_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetSubnetworkRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.Subnetwork(
            ...
            kind='kind_value',
            log_config=compute.SubnetworkLogConfig(aggregation_interval=compute.SubnetworkLogConfig.AggregationInterval.INTERVAL_10_MIN),
            name='name_value',
            ...
            secondary_ip_ranges=[compute.SubnetworkSecondaryRange(ip_cidr_range='ip_cidr_range_value')],
            self_link='self_link_value',
    ...
    assert response.kind == 'kind_value'
    assert response.log_config == compute.SubnetworkLogConfig(aggregation_interval=compute.SubnetworkLogConfig.AggregationInterval.INTERVAL_10_MIN)
    assert response.name == 'name_value'
    ...
    assert response.secondary_ip_ranges == [compute.SubnetworkSecondaryRange(ip_cidr_range='ip_cidr_range_value')]
    assert response.self_link == 'self_link_value'
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.insert(
project='project_value',region='region_value',subnetwork_resource=subnetwork_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'region_value'
 in http_call[1] + str(body)
        assert compute.Subnetwork.to_json(subnetwork_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListSubnetworksRequest):
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.Subnetwork(creation_timestamp='creation_timestamp_value')]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_list_usable_rest(transport: str = 'rest', request_type=compute.ListUsableSubnetworksRequest):
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.UsableSubnetwork(ip_cidr_range='ip_cidr_range_value')]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_patch_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        subnetwork_resource = compute.Subnetwork(creation_timestamp='creation_timestamp_value')
        client.patch(
project='project_value',region='region_value',subnetwork='subnetwork_value',subnetwork_resource=subnetwork_resource,        )
...
project='project_value',region='region_value',subnetwork='subnetwork_value',subnetworks_set_private_ip_google_access_request_resource=subnetworks_set_private_ip_google_access_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'subnetwork_value'
 in http_call[1] + str(body)
        assert compute.SubnetworksSetPrivateIpGoogleAccessRequest.to_json(subnetworks_set_private_ip_google_access_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_subnetworks_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.subnetworks.transports.SubnetworksTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_subnetworks_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.subnetworks.transports.SubnetworksTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.SubnetworksTransport()
        adc.assert_called_once()
