...
from google.cloud.compute_v1.services.licenses.transports.base import _GOOGLE_AUTH_VERSION
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetLicenseRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.License(
            ...
            name='name_value',
            resource_requirements=compute.LicenseResourceRequirements(min_guest_cpu_count=2042),
            self_link='self_link_value',
            transferable=True,
        )
    ...
    assert response.name == 'name_value'
    assert response.resource_requirements == compute.LicenseResourceRequirements(min_guest_cpu_count=2042)
    assert response.self_link == 'self_link_value'
...
project='project_value',license_resource=license_resource,        )
        ...
        assert 'project_value'
 in http_call[1] + str(body)
        assert compute.License.to_json(license_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_licenses_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.licenses.transports.LicensesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_licenses_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.licenses.transports.LicensesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.LicensesTransport()
        adc.assert_called_once()
