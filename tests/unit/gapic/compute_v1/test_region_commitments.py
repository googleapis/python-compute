...
from google.cloud.compute_v1.services.region_commitments.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.region_commitments.transports.base import _GOOGLE_AUTH_VERSION
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListRegionCommitmentsRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.CommitmentAggregatedList(
            id='id_value',
            items={'key_value': compute.CommitmentsScopedList(commitments=[compute.Commitment(category=compute.Commitment.Category.CATEGORY_UNSPECIFIED)])},
            kind='kind_value',
    ...
    assert response.items == {'key_value': compute.CommitmentsScopedList(commitments=[compute.Commitment(category=compute.Commitment.Category.CATEGORY_UNSPECIFIED)])}
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetRegionCommitmentRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.Commitment(
            ...
            resources=[compute.ResourceCommitment(accelerator_type='accelerator_type_value')],
            self_link='self_link_value',
    ...
    assert response.id == 'id_value'
    assert response.kind == 'kind_value'
    assert response.license_resource == compute.LicenseResourceCommitment(amount='amount_value')
    ...
    assert response.resources == [compute.ResourceCommitment(accelerator_type='accelerator_type_value')]
    assert response.self_link == 'self_link_value'
...
def test_get_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # using the keyword arguments to the method.
        client.get(
project='project_value',region='region_value',commitment='commitment_value',        )
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.insert(
project='project_value',region='region_value',commitment_resource=commitment_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'region_value'
 in http_call[1] + str(body)
        assert compute.Commitment.to_json(commitment_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListRegionCommitmentsRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.CommitmentList(
            id='id_value',
            items=[compute.Commitment(category=compute.Commitment.Category.CATEGORY_UNSPECIFIED)],
            kind='kind_value',
            next_page_token='next_page_token_value',
            self_link='self_link_value',
            warning=compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED),
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.Commitment(category=compute.Commitment.Category.CATEGORY_UNSPECIFIED)]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_region_commitments_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.region_commitments.transports.RegionCommitmentsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_region_commitments_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.region_commitments.transports.RegionCommitmentsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.RegionCommitmentsTransport()
        adc.assert_called_once()
