...
def test_region_commitments_client_from_service_account_info():
    ...
    with mock.patch.object(
        ...
        info = {"valid": True}
        client = RegionCommitmentsClient.from_service_account_info(info)
...
def test_aggregated_list_rest_flattened():
    client = RegionCommitmentsClient(credentials=credentials.AnonymousCredentials(),)
...
def test_get_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        assert "commitment_value" in http_call[1] + str(body)
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        assert compute.Commitment.to_json(
            commitment_resource, including_default_value_fields=False
