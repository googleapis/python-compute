...
from google.cloud.compute_v1.services.machine_types.transports.base import (
...
@pytest.mark.parametrize("client_class", [MachineTypesClient,])
def test_machine_types_client_service_account_always_use_jwt(client_class):
    with mock.patch.object(
...
def test_machine_types_client_service_account_always_use_jwt_true(
...
def test_aggregated_list_rest(
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        return_value = compute.MachineTypeAggregatedList(
            id="id_value",
            items={
                "key_value": compute.MachineTypesScopedList(
                    machine_types=[
                        compute.MachineType(
                            accelerators=[
                                compute.Accelerators(guest_accelerator_count=2452)
                            ]
                        )
                    ]
                )
