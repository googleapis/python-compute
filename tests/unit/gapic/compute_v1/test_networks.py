...
from google.cloud.compute_v1.services.networks.transports.base import _GOOGLE_AUTH_VERSION
...
def test_add_peering_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.add_peering(
project='project_value',network='network_value',networks_add_peering_request_resource=networks_add_peering_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'network_value'
 in http_call[1] + str(body)
        assert compute.NetworksAddPeeringRequest.to_json(networks_add_peering_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetNetworkRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.Network(
            ...
            mtu=342,
            name='name_value',
            peerings=[compute.NetworkPeering(auto_create_routes=True)],
            routing_config=compute.NetworkRoutingConfig(routing_mode=compute.NetworkRoutingConfig.RoutingMode.GLOBAL),
            self_link='self_link_value',
    ...
    assert response.name == 'name_value'
    assert response.peerings == [compute.NetworkPeering(auto_create_routes=True)]
    assert response.routing_config == compute.NetworkRoutingConfig(routing_mode=compute.NetworkRoutingConfig.RoutingMode.GLOBAL)
    assert response.self_link == 'self_link_value'
...
project='project_value',network_resource=network_resource,        )
        ...
        assert 'project_value'
 in http_call[1] + str(body)
        assert compute.Network.to_json(network_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_peering_routes_rest(transport: str = 'rest', request_type=compute.ListPeeringRoutesNetworksRequest):
    client = NetworksClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.ExchangedPeeringRoutesList(
            id='id_value',
            items=[compute.ExchangedPeeringRoute(dest_range='dest_range_value')],
            kind='kind_value',
            next_page_token='next_page_token_value',
            self_link='self_link_value',
            warning=compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED),
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.ExchangedPeeringRoute(dest_range='dest_range_value')]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_list_peering_routes_rest_from_dict():
    test_list_peering_routes_rest(request_type=dict)
...
def test_patch_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        network_resource = compute.Network(auto_create_subnetworks=True)
        client.patch(
project='project_value',network='network_value',network_resource=network_resource,        )
...
project='project_value',network='network_value',networks_remove_peering_request_resource=networks_remove_peering_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'network_value'
 in http_call[1] + str(body)
        assert compute.NetworksRemovePeeringRequest.to_json(networks_remove_peering_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
project='project_value',network='network_value',networks_update_peering_request_resource=networks_update_peering_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'network_value'
 in http_call[1] + str(body)
        assert compute.NetworksUpdatePeeringRequest.to_json(networks_update_peering_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_networks_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.networks.transports.NetworksTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_networks_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.networks.transports.NetworksTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.NetworksTransport()
        adc.assert_called_once()
