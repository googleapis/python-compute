...
@pytest.mark.parametrize("client_class", [NetworksClient,])
def test_networks_client_service_account_always_use_jwt(client_class):
    with mock.patch.object(
...
def test_networks_client_service_account_always_use_jwt_true(
...
def test_add_peering_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        client.add_peering(
            ...
            networks_add_peering_request_resource=networks_add_peering_request_resource,
        )
...
def test_get_rest(transport: str = "rest", request_type=compute.GetNetworkRequest):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        return_value = compute.Network(
            ...
            routing_config=compute.NetworkRoutingConfig(
                routing_mode=compute.NetworkRoutingConfig.RoutingMode.GLOBAL
            ),
            self_link="self_link_value",
    ...
    assert response.routing_config == compute.NetworkRoutingConfig(
        routing_mode=compute.NetworkRoutingConfig.RoutingMode.GLOBAL
    )
    assert response.self_link == "self_link_value"
