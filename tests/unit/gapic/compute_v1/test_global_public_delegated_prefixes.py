...
from google.cloud.compute_v1.services.global_public_delegated_prefixes.transports.base import (
...
def test_global_public_delegated_prefixes_client_service_account_always_use_jwt_true(
    transport_class, transport_name
):
    with mock.patch.object(
        service_account.Credentials, "with_always_use_jwt_access", create=True
...
def test_get_rest(
    ...
    assert response.public_delegated_sub_prefixs == [
        compute.PublicDelegatedPrefixPublicDelegatedSubPrefix(
            delegatee_project="delegatee_project_value"
        )
    ]
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        client.insert(
            project="project_value",
            public_delegated_prefix_resource=public_delegated_prefix_resource,
        )
...
def test_list_rest(
    ...
    assert response.items == [
        compute.PublicDelegatedPrefix(creation_timestamp="creation_timestamp_value")
    ]
    assert response.kind == "kind_value"
...
def test_global_public_delegated_prefixes_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(
        google.auth, "load_credentials_from_file", autospec=True
    ) as load_creds, mock.patch(
...
@requires_google_auth_lt_1_25_0
def test_global_public_delegated_prefixes_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        GlobalPublicDelegatedPrefixesClient()
