...
def test_url_maps_client_from_service_account_info():
    ...
    with mock.patch.object(
        ...
        info = {"valid": True}
        client = UrlMapsClient.from_service_account_info(info)
...
def test_aggregated_list_rest(
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        return_value = compute.UrlMapsAggregatedList(
            id="id_value",
            items={
                "key_value": compute.UrlMapsScopedList(
                    url_maps=[
    ...
    assert response.items == {
        "key_value": compute.UrlMapsScopedList(
            url_maps=[compute.UrlMap(creation_timestamp="creation_timestamp_value")]
        )
    }
...
def test_invalidate_cache_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        client.invalidate_cache(
            project="project_value",
            url_map="url_map_value",
            cache_invalidation_rule_resource=cache_invalidation_rule_resource,
        )
        ...
        assert compute.CacheInvalidationRule.to_json(
            cache_invalidation_rule_resource, including_default_value_fields=False
        ) in http_call[1] + str(body)
...
def test_validate_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        client.validate(
            project="project_value",
            url_map="url_map_value",
            url_maps_validate_request_resource=url_maps_validate_request_resource,
        )
        ...
        assert compute.UrlMapsValidateRequest.to_json(
            url_maps_validate_request_resource, including_default_value_fields=False
        ) in http_call[1] + str(body)
