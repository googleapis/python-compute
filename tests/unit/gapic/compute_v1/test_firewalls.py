...
from google.cloud.compute_v1.services.firewalls.transports.base import _GOOGLE_AUTH_VERSION
...
project='project_value',firewall_resource=firewall_resource,        )
        ...
        assert 'project_value'
 in http_call[1] + str(body)
        assert compute.Firewall.to_json(firewall_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListFirewallsRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.FirewallList(
            id='id_value',
            items=[compute.Firewall(allowed=[compute.Allowed(i_p_protocol='i_p_protocol_value')])],
            kind='kind_value',
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.Firewall(allowed=[compute.Allowed(i_p_protocol='i_p_protocol_value')])]
    assert response.kind == 'kind_value'
...
def test_patch_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        firewall_resource = compute.Firewall(allowed=[compute.Allowed(i_p_protocol='i_p_protocol_value')])
        client.patch(
project='project_value',firewall='firewall_value',firewall_resource=firewall_resource,        )
...
def test_firewalls_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.firewalls.transports.FirewallsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_firewalls_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.firewalls.transports.FirewallsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.FirewallsTransport()
        adc.assert_called_once()
