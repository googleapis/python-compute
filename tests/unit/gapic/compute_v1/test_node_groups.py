...
from google.cloud.compute_v1.services.node_groups.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.node_groups.transports.base import _GOOGLE_AUTH_VERSION
...
def test_add_nodes_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.add_nodes(
project='project_value',zone='zone_value',node_group='node_group_value',node_groups_add_nodes_request_resource=node_groups_add_nodes_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'zone_value'
 in http_call[1] + str(body)
        assert 'node_group_value'
 in http_call[1] + str(body)
        assert compute.NodeGroupsAddNodesRequest.to_json(node_groups_add_nodes_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListNodeGroupsRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeGroupAggregatedList(
            id='id_value',
            items={'key_value': compute.NodeGroupsScopedList(node_groups=[compute.NodeGroup(autoscaling_policy=compute.NodeGroupAutoscalingPolicy(max_nodes=958))])},
            kind='kind_value',
    ...
    assert response.id == 'id_value'
    assert response.items == {'key_value': compute.NodeGroupsScopedList(node_groups=[compute.NodeGroup(autoscaling_policy=compute.NodeGroupAutoscalingPolicy(max_nodes=958))])}
    assert response.kind == 'kind_value'
...
def test_delete_nodes_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.delete_nodes(
project='project_value',zone='zone_value',node_group='node_group_value',node_groups_delete_nodes_request_resource=node_groups_delete_nodes_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'node_group_value'
 in http_call[1] + str(body)
        assert compute.NodeGroupsDeleteNodesRequest.to_json(node_groups_delete_nodes_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetNodeGroupRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeGroup(
            ...
            maintenance_window=compute.NodeGroupMaintenanceWindow(maintenance_duration=compute.Duration(nanos=543)),
            name='name_value',
    ...
    assert response.autoscaling_policy == compute.NodeGroupAutoscalingPolicy(max_nodes=958)
    assert response.creation_timestamp == 'creation_timestamp_value'
    assert response.description == 'description_value'
    ...
    assert response.maintenance_window == compute.NodeGroupMaintenanceWindow(maintenance_duration=compute.Duration(nanos=543))
    assert response.name == 'name_value'
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.insert(
project='project_value',zone='zone_value',initial_node_count=1911,node_group_resource=node_group_resource,        )
 ...
 in http_call[1] + str(body)
        assert str(1911)
 in http_call[1] + str(body)
        assert compute.NodeGroup.to_json(node_group_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListNodeGroupsRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeGroupList(
            id='id_value',
            items=[compute.NodeGroup(autoscaling_policy=compute.NodeGroupAutoscalingPolicy(max_nodes=958))],
            kind='kind_value',
            next_page_token='next_page_token_value',
            self_link='self_link_value',
            warning=compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED),
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.NodeGroup(autoscaling_policy=compute.NodeGroupAutoscalingPolicy(max_nodes=958))]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_list_nodes_rest(transport: str = 'rest', request_type=compute.ListNodesNodeGroupsRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeGroupsListNodes(
            id='id_value',
            items=[compute.NodeGroupNode(accelerators=[compute.AcceleratorConfig(accelerator_count=1805)])],
            kind='kind_value',
            next_page_token='next_page_token_value',
            self_link='self_link_value',
            warning=compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED),
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.NodeGroupNode(accelerators=[compute.AcceleratorConfig(accelerator_count=1805)])]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_patch_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        node_group_resource = compute.NodeGroup(autoscaling_policy=compute.NodeGroupAutoscalingPolicy(max_nodes=958))
        client.patch(
project='project_value',zone='zone_value',node_group='node_group_value',node_group_resource=node_group_resource,        )
...
project='project_value',zone='zone_value',node_group='node_group_value',node_groups_set_node_template_request_resource=node_groups_set_node_template_request_resource,        )
 ...
 in http_call[1] + str(body)
        assert 'node_group_value'
 in http_call[1] + str(body)
        assert compute.NodeGroupsSetNodeTemplateRequest.to_json(node_groups_set_node_template_request_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_node_groups_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.node_groups.transports.NodeGroupsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_node_groups_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.node_groups.transports.NodeGroupsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.NodeGroupsTransport()
        adc.assert_called_once()
