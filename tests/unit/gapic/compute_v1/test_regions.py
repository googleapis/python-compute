...
from google.cloud.compute_v1.services.regions.transports.base import _GOOGLE_AUTH_VERSION
...
def test_regions_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListRegionsRequest):
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.Region(creation_timestamp='creation_timestamp_value')]
    assert response.kind == 'kind_value'
...
def test_regions_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.regions.transports.RegionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_regions_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.regions.transports.RegionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.RegionsTransport()
        adc.assert_called_once()
