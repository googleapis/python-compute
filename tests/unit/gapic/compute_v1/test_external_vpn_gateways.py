...
from google.cloud.compute_v1.services.external_vpn_gateways.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.external_vpn_gateways.transports.base import _GOOGLE_AUTH_VERSION
...
project='project_value',external_vpn_gateway='external_vpn_gateway_value',        )
 ...
 in http_call[1] + str(body)
        assert 'external_vpn_gateway_value'
 in http_call[1] + str(body)
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        client.insert(
project='project_value',external_vpn_gateway_resource=external_vpn_gateway_resource,        )
        ...
        assert 'project_value'
 in http_call[1] + str(body)
        assert compute.ExternalVpnGateway.to_json(external_vpn_gateway_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListExternalVpnGatewaysRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.ExternalVpnGatewayList(
            etag='etag_value',
            id='id_value',
            items=[compute.ExternalVpnGateway(creation_timestamp='creation_timestamp_value')],
            kind='kind_value',
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.ExternalVpnGateway(creation_timestamp='creation_timestamp_value')]
    assert response.kind == 'kind_value'
...
def test_external_vpn_gateways_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.external_vpn_gateways.transports.ExternalVpnGatewaysTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_external_vpn_gateways_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.external_vpn_gateways.transports.ExternalVpnGatewaysTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.ExternalVpnGatewaysTransport()
        adc.assert_called_once()
