...
from google.cloud.compute_v1.services.node_types.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.node_types.transports.base import _GOOGLE_AUTH_VERSION
...
def test_node_types_client_from_service_account_info(client_class):
...
def test_node_types_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_aggregated_list_rest(transport: str = 'rest', request_type=compute.AggregatedListNodeTypesRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeTypeAggregatedList(
            id='id_value',
            items={'key_value': compute.NodeTypesScopedList(node_types=[compute.NodeType(cpu_platform='cpu_platform_value')])},
            kind='kind_value',
    ...
    assert response.items == {'key_value': compute.NodeTypesScopedList(node_types=[compute.NodeType(cpu_platform='cpu_platform_value')])}
...
def test_get_rest(transport: str = 'rest', request_type=compute.GetNodeTypeRequest):
    client = NodeTypesClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListNodeTypesRequest):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        return_value = compute.NodeTypeList(
            id='id_value',
            items=[compute.NodeType(cpu_platform='cpu_platform_value')],
            kind='kind_value',
            next_page_token='next_page_token_value',
            self_link='self_link_value',
            warning=compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED),
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.NodeType(cpu_platform='cpu_platform_value')]
    assert response.kind == 'kind_value'
    assert response.next_page_token == 'next_page_token_value'
    assert response.self_link == 'self_link_value'
    assert response.warning == compute.Warning(code=compute.Warning.Code.CLEANUP_FAILED)
...
def test_node_types_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.node_types.transports.NodeTypesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_node_types_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.node_types.transports.NodeTypesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (credentials.AnonymousCredentials(), None)
...
def test_node_types_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.node_types.transports.NodeTypesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.NodeTypesTransport()
        adc.assert_called_once()
...
def test_node_types_host_no_port():
    client = NodeTypesClient(
        credentials=credentials.AnonymousCredentials(),
