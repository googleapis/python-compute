...
from google.cloud.compute_v1.services.ssl_policies.transports.base import _API_CORE_VERSION
from google.cloud.compute_v1.services.ssl_policies.transports.base import _GOOGLE_AUTH_VERSION
...
project='project_value',ssl_policy_resource=ssl_policy_resource,        )
        ...
        assert 'project_value'
 in http_call[1] + str(body)
        assert compute.SslPolicy.to_json(ssl_policy_resource, including_default_value_fields=False, use_integers_for_enums=False)
 in http_call[1] + str(body)
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListSslPoliciesRequest):
    ...
    assert response.id == 'id_value'
    assert response.items == [compute.SslPolicy(creation_timestamp='creation_timestamp_value')]
    assert response.kind == 'kind_value'
...
def test_patch_rest_flattened():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        ssl_policy_resource = compute.SslPolicy(creation_timestamp='creation_timestamp_value')
        client.patch(
project='project_value',ssl_policy='ssl_policy_value',ssl_policy_resource=ssl_policy_resource,        )
...
def test_ssl_policies_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.ssl_policies.transports.SslPoliciesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_ssl_policies_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.ssl_policies.transports.SslPoliciesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.SslPoliciesTransport()
        adc.assert_called_once()
