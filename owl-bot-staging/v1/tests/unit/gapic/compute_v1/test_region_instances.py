...
def test_region_instances_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_bulk_insert_rest(transport: str = 'rest', request_type=compute.BulkInsertRegionInstanceRequest):
...
def test_bulk_insert_rest_flattened(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert path_template.validate("https://%s/compute/v1/projects/{project}/regions/{region}/instances/bulkInsert" % client.transport._host, args[1])
