...
def test_set_named_ports_unary_rest(transport: str = 'rest', request_type=compute.SetNamedPortsRegionInstanceGroupRequest):
    client = RegionInstanceGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "region": "sample2", "instance_group": "sample3"}
    request_init["region_instance_groups_set_named_ports_request_resource"] = compute.RegionInstanceGroupsSetNamedPortsRequest(fingerprint='fingerprint_value')
    request = request_type(request_init)
