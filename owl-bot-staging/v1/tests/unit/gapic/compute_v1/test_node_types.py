...
def test_aggregated_list_rest(request_type, transport: str = 'rest'):
    ...
    request = compute.AggregatedListNodeTypesRequest({'project': 'sample1'})
...
def test_get_rest(request_type, transport: str = 'rest'):
    ...
    # Send a request that will satisfy transcoding
    request = compute.GetNodeTypeRequest({'project': 'sample1', 'zone': 'sample2', 'node_type': 'sample3'})
...
def test_get_rest_required_fields(request_type=compute.GetNodeTypeRequest):
    ...
    # verify fields with default values are dropped
    assert "nodeType" not in jsonified_request
    assert "project" not in jsonified_request
    ...
    assert jsonified_request["nodeType"] == request_init["node_type"]
    assert "project" in jsonified_request
    assert jsonified_request["project"] == request_init["project"]
...
def test_list_rest(request_type, transport: str = 'rest'):
    ...
    # Send a request that will satisfy transcoding
    request = compute.ListNodeTypesRequest({'project': 'sample1', 'zone': 'sample2'})
