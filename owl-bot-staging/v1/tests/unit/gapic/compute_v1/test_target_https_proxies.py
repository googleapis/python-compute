...
def test_aggregated_list_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        pb_return_value = compute.TargetHttpsProxyAggregatedList.pb(return_value)
...
def test_get_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = compute.TargetHttpsProxy.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
...
def test_insert_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'project': 'sample1'}
    request_init["target_https_proxy_resource"] = {'authorization_policy': 'authorization_policy_value', 'certificate_map': 'certificate_map_value', 'creation_timestamp': 'creation_timestamp_value', 'description': 'description_value', 'fingerprint': 'fingerprint_value', 'id': 205, 'kind': 'kind_value', 'name': 'name_value', 'proxy_bind': True, 'quic_override': 'quic_override_value', 'region': 'region_value', 'self_link': 'self_link_value', 'server_tls_policy': 'server_tls_policy_value', 'ssl_certificates': ['ssl_certificates_value1', 'ssl_certificates_value2'], 'ssl_policy': 'ssl_policy_value', 'url_map': 'url_map_value'}
    request = request_type(**request_init)
...
def test_list_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = compute.TargetHttpsProxyList.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
...
def test_set_certificate_map_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TargetHttpsProxiesRestInterceptor, "post_set_certificate_map") as post, \
         mock.patch.object(transports.TargetHttpsProxiesRestInterceptor, "pre_set_certificate_map") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_set_certificate_map_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        mock_args = dict(
            project='project_value',
            target_https_proxy='target_https_proxy_value',
            target_https_proxies_set_certificate_map_request_resource=compute.TargetHttpsProxiesSetCertificateMapRequest(certificate_map='certificate_map_value'),
        )
        mock_args.update(sample_request)
...
def test_transport_kind(transport_name):
    transport = TargetHttpsProxiesClient.get_transport_class(transport_name)(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
def test_target_https_proxies_client_transport_session_collision(transport_name):
    ...
    client1 = TargetHttpsProxiesClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = TargetHttpsProxiesClient(
        credentials=creds2,
        transport=transport_name,
    )
