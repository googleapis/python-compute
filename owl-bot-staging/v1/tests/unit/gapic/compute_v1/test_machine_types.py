...
def test_aggregated_list_rest(request_type, transport: str = 'rest'):
    ...
    request = compute.AggregatedListMachineTypesRequest({'project': 'sample1'})
...
def test_get_rest(request_type, transport: str = 'rest'):
    ...
    # Send a request that will satisfy transcoding
    request = compute.GetMachineTypeRequest({'project': 'sample1', 'zone': 'sample2', 'machine_type': 'sample3'})
...
def test_get_rest_required_fields(request_type=compute.GetMachineTypeRequest):
    ...
    # verify fields with default values are dropped
    assert "machineType" not in jsonified_request
    assert "project" not in jsonified_request
    ...
    assert jsonified_request["machineType"] == request_init["machine_type"]
    assert "project" in jsonified_request
    assert jsonified_request["project"] == request_init["project"]
...
def test_list_rest(request_type, transport: str = 'rest'):
    ...
    # Send a request that will satisfy transcoding
    request = compute.ListMachineTypesRequest({'project': 'sample1', 'zone': 'sample2'})
