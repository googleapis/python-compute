...
def test_get_rest_interceptors(null_interceptor):
    transport = transports.LicenseCodesRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.LicenseCodesRestInterceptor(),
        )
    client = LicenseCodesClient(transport=transport)
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.LicenseCodesRestInterceptor, "post_get") as post, \
         mock.patch.object(transports.LicenseCodesRestInterceptor, "pre_get") as pre:
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.LicenseCodesRestInterceptor, "post_test_iam_permissions") as post, \
         mock.patch.object(transports.LicenseCodesRestInterceptor, "pre_test_iam_permissions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
