...
class GlobalAddressesClient(metaclass=GlobalAddressesClientMeta):
    ...
    def __init__(self, *,
        ...
        if isinstance(transport, GlobalAddressesTransport):
    ...
    def move_unary(self,
            ...
            *,
            project: Optional[str] = None,
            address: Optional[str] = None,
            global_addresses_move_request_resource: Optional[compute.GlobalAddressesMoveRequest] = None,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
            timeout: Union[float, object] = gapic_v1.method.DEFAULT,
            metadata: Sequence[Tuple[str, str]] = (),
            ) -> compute.Operation:
        ...
        # Quick check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([project, address, global_addresses_move_request_resource])
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
        ...
        if not isinstance(request, compute.MoveGlobalAddressRequest):
            ...
            if global_addresses_move_request_resource is not None:
                request.global_addresses_move_request_resource = global_addresses_move_request_resource
