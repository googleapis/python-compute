...
class ForwardingRulesClient(metaclass=ForwardingRulesClientMeta):
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, ForwardingRulesTransport]] = None,
    ...
    def patch_unary(self,
        ...
        # Quick check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([project, region, forwarding_rule, forwarding_rule_resource])
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
    ...
    def set_target_unary(self,
        ...
        # Quick check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([project, region, forwarding_rule, target_reference_resource])
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
