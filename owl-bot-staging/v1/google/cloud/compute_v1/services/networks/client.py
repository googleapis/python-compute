...
class NetworksClient(metaclass=NetworksClientMeta):
    ...
    def add_peering_unary(self,
        ...
        has_flattened_params = any([project, network, networks_add_peering_request_resource])
    ...
    def remove_peering_unary(self,
        ...
        # Sanity check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([project, network, networks_remove_peering_request_resource])
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
