...
class TargetTcpProxiesClient(metaclass=TargetTcpProxiesClientMeta):
    ...
    def set_backend_service_unary(self,
        ...
        # Sanity check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([project, target_tcp_proxy, target_tcp_proxies_set_backend_service_request_resource])
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
    ...
    def set_proxy_header_unary(self,
        ...
        # Sanity check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([project, target_tcp_proxy, target_tcp_proxies_set_proxy_header_request_resource])
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
