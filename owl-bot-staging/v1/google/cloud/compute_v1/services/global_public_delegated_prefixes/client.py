...
class GlobalPublicDelegatedPrefixesClient(metaclass=GlobalPublicDelegatedPrefixesClientMeta):
    ...
    def list(self,
            request: Union[compute.ListGlobalPublicDelegatedPrefixesRequest, dict] = None,
            *,
            project: str = None,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
            timeout: float = None,
    ...
    def patch_unary(self,
        ...
        # Sanity check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([project, public_delegated_prefix, public_delegated_prefix_resource])
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
