...
class SubnetworksClient(metaclass=SubnetworksClientMeta):
    ...
    def expand_ip_cidr_range_unary(self,
    ...
    def set_private_ip_google_access_unary(self,
        ...
        # Sanity check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([project, region, subnetwork, subnetworks_set_private_ip_google_access_request_resource])
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
